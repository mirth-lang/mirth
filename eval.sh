...
#ls src | grep .mth | sed 's/.mth/\)/' | sed 's/^/import\(mirth./' >> $PKG/main.mth
echo "def(main,+World -- +World," >> $PKG/main.mth
echo $* >> $PKG/main.mth
echo "?? 0 prim-posix-exit)" >> $PKG/main.mth
bin/mirth2 -p std:lib/std -p arg-parser:lib/arg-parser -p mirth:src -p eval:$PKG $PKG/main.mth -o $TMP/main.c
